...
int main() {
	...
	try {
		...
		cout << "Переповнення стека: " << exc.what() << endl;
	}
	catch (stack_bad_alloc& exc) {
		cout << "Помилка виділення пам'яті для елемента стека." << endl;
	}
	catch (stack_size_error& exc) {
		cout << "Помилка розміру стека: " << exc.what() << endl;
	}
	catch (stack_empty& exc) {
		cout << "Стек порожній: " << exc.what() << endl;
	}
